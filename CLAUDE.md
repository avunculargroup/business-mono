...
|The Service Statement, the information-only notice, or the disclosure gate|`docs/features/client-app/compliance/service-statement.md` + `supabase/migrations/20260911010000_compliance_documents.sql`|The gate serves a **Service Statement**, never an FSG — `doc_type`'s CHECK constraint rejects `fsg`, because publishing one would imply an authorisation BTS does not hold. The standing notice is an **information-only notice**, never a "general advice warning", which implies licensed general advice. The statement is seeded as a **draft** and its `{{variables}}` resolve from `company_profile` at render, so it cannot go active until that row is filled — and its text makes claims the app must keep true (no facility for personal circumstances, prose never transmitted, every fact carries a link), which `apps/client/lib/serviceStatement.test.ts` asserts|
