...
|Touching data access on an existing `apps/web` page|`docs/features/demo-app/repository-contract.md`|New access goes through `@platform/data`; existing direct `.from(` calls stay until their vertical is done — ~267 across ~85 files remain from deferred verticals, and mixing the two on one page is expected. Every page moved onto a repository is a page `apps/demo` tracks for free; every page left off is one it cannot|
