...
|The Service Statement, the information-only notice, or the disclosure gate|`supabase/migrations/20260911010000_compliance_documents.sql`|The gate serves a **Service Statement**, never an FSG — `doc_type`'s CHECK constraint rejects `fsg`, because publishing one would imply an authorisation BTS does not hold. The standing notice is an **information-only notice**, never a "general advice warning", which implies licensed general advice|
