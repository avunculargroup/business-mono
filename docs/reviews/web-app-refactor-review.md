...
**Date:** 2026-07-16 · **Status:** in progress — all three P1 items shipped; P2 abstractions landed (items 4–7) with representative migrations, remaining migrations incremental; P3 open (see Progress log).
...
| 4. Shared form hook + field components | ◐ in progress | 2026-07-17 — built the kit: `hooks/useEntityForm.ts` (owns the create/edit branch, toast, `useActionState`, and the `onPendingChange` relay) and `components/ui/FormField.tsx` (`FormField`/`FormRow`/`FormSelect`/`FormTextarea`/`FormError`, each wiring `htmlFor`/`id` + `aria-describedby`/`aria-invalid` — closes the unassociated-label a11y gap). Tested (`FormField.test.tsx`). Migrated 3 forms as proof (Contact, Company, Segment). Remaining ~23 `*Form.tsx` migrate onto the kit incrementally. |
...
| 6. Shared CRUD list-page scaffold | ◐ in progress | 2026-07-17 — added `hooks/useEntityList.ts`: owns the create/edit/delete dialog-state triad, the async delete flow (confirm → action → toast → refresh), and an optimistic list (composes `useOptimisticList`). Tested (`useEntityList.test.tsx`). Migrated CompaniesList + ContactsList onto it, and moved the hand-rolled ChampionsList onto `useOptimisticList` (the doc's explicit ask). Remaining lists (Personas, Interviews, Segments, discovery/Feedback, tasks/TasksView) + the other hand-roller (discovery/PipelineBoard) migrate incrementally. |
