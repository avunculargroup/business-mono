...
| 10. Decompose the three giant client views | ◐ in progress | 2026-07-17 — extracted the shared `components/ui/TagInput` (the review's named `ui/TagInput`), unifying the chip inputs hand-rolled across InterviewForm, FeedbackForm, TemplateForm, and CommunityForm (which had its own local duplicate). Tested (`TagInput.test.tsx`); deleted the now-unused `crm/InterviewForm.module.css`. **Remaining:** the FilesView/PodcastDashboard/RoutineForm decompositions and adopting `ui/TagInput` in FilesView's `TagInput` + RoutineForm's `ChipInput`. |
