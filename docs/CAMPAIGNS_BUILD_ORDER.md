...
| 10 — Optional branches | ⚠️ **built; needs a live pass** | Rex (research) + Bruno (audience) wired as conditional steps BEFORE strategy synthesis (`research`, `audience` in `workflows/strategy/index.ts`). Conditions are pure heuristics (`shouldRunResearch` = current-events/competitor/trend/recent-year signal in the objective; `shouldRunAudienceAnalysis` = a real industry/pipeline segment), so no LLM gate and no cost when not warranted. Both branches are best-effort (failure → '' , run continues) and their output folds into the synthesis prompt; Bruno gets a light CRM company lookup by industry. Step outputs are cached in the run snapshot, so gate resumes never re-run them. `strategy.research` (fallback rex) + `strategy.audience` (fallback bruno) added to `MODEL_SCOPES`. 551 agents tests green. **Remaining:** live pass. |
| 9 — Loops & polish | ⚠️ **built end-to-end; needs a live pass** | **Compliance re-run on edit** (application layer): `editVariantCopy` action resets a variant to `compliance_status='pending'` + patches the suspended `gate_state` preview; the `complianceRecheck` listener (atomic claim on `compliance_checked_at IS NULL`) re-invokes Lex via the variant classifier and rewrites the compliance fields + preview. Inline copy editing added to the variant editor (single post + thread segments). **Metrics entry**: `savePostMetrics` upsert + inline platform-aware fields on published posts (`PublishedPosts`). **Promote-from-post**: `promotePostToSnippet` → `voice_snippets` (`source=promoted_from_post`, `source_content_item_id`, founder curator note) on published posts. No new migration (reuses `post_metrics`, `voice_snippets`). Typecheck/lint clean; 543 agents + 125 web tests green. **Remaining:** live pass (edit a cleared variant → Lex re-runs; log metrics; promote a post → it appears in Brand Hub snippets once embedded). |
| 8 — Fan-out + matrix + queue | ⚠️ **built end-to-end; needs a live pass** | `workflows/strategy/fanOut.ts` spawns one variant run per `schedule_plan` entry (fire-and-track, sequential in the background), atomically claiming `plan_approved → active` so it can't double-spawn; triggered from `resumeStrategyRun` after Gate 2. `startVariantRun` added to `variant/run.ts`; each spawned variant's `scheduled_for` is stamped from the plan. Web: `CampaignMatrix` (agenda + coverage-grid toggle, from `v_campaign_matrix`) on the campaign detail, and the ready-to-post queue (`/campaigns/[id]/queue`, `ReadyToPostQueue` from `v_ready_to_post` + thread segments) with copy-out / copy-by-segment / mark-as-posted (`markVariantPosted` → `published_url` + `status=published`). No new migration (reuses Step 4 views + `content_items.scheduled_for`). Typecheck/lint clean; 536 agents tests green. **Remaining:** live pass (real plan approval → variants appear in the matrix → approve → post). |
| 7 — Campaign Strategy Workflow | ⚠️ **built end-to-end; needs a live pass** | Workflow `apps/agents/src/workflows/strategy/` (resolve-context → Margot synthesis → **Gate 1** → Margot beat plan + deterministic `schedule.ts` → **Gate 2** → persist beats + `schedule_plan`, lock at `plan_approved`). Two-gate resume targeting via `inspectSuspendedGate` (mirrors the newsletter). Migration `20260623000000` adds campaign gate columns. `strategyGateWeb` listener launches (`{decision:'start'}`) + resumes both gates. `strategy.synthesise`/`strategy.plan_beats` in `MODEL_SCOPES` (+ `WORKFLOW_LABELS`). **Margot wired onto Simon's roster** (`simon.agents` + routing line + `agent-margot` + a campaign fixture in `simon-routing.eval.ts`). Web: creation wizard + list + canvas/2-gate panels (`CampaignWizard`, `CampaignWorkspace`, `app/(app)/campaigns/{page,new,[id]}`), `campaigns.ts` server actions, sidebar nav. agents + web + shared typecheck clean; 534 agents + 125 web tests green. **Remaining:** live pass (Studio run + a real wizard run with secrets); the editable-canvas-after-creation strategy editor is read-only/locked-only for now (edit-before-lock happens at the gates). |
...
- ~~`posts_per_week` per-account vs total (Step 7 scheduling)~~ — **settled: total across accounts** (Phase-1 default). `schedule.ts` distributes (beat × account) across slots over the duration; surplus beyond `posts_per_week × duration_weeks` is planned but left unscheduled. The approved plan persists to `campaigns.schedule_plan` for Step 8 fan-out (chosen over recomputing at fan-out). Strategy lock is application-layer (server actions reject gate decisions past `strategy_approved`).