...
| 6 — Variant Generation Workflow | ⚠️ **workflow spine done; editor UI pending** | `apps/agents/src/workflows/variant/` — resolve-context → Charlie → Lex → persist `content_item` draft → **Gate 3 (suspend)**, with single-variant regeneration and X threads. Registered in Mastra `workflows:` map + `MODEL_SCOPES` (`variant.generate_copy`/`variant.compliance_check`). Pure helpers (prompts/persist/schemas) unit-tested — 277 tests green, typecheck clean. **Remaining:** the variant editor UI (sub-part 3) and a live Studio end-to-end pass (needs secrets + a stub campaign/beat row). |
...
**Next:** finish **Step 6** by building the **variant editor UI** that drives Gate 3 (platform-mimic preview, live char counter, Lex compliance chip, resolved voice, image slot + alt text, inline approve / request-change) — the suspend payload `variantGateSuspendSchema.preview` is the render contract. Then a live Studio end-to-end pass (insert a stub `campaigns` + `campaign_beats` row; `social_accounts` + `platform_specs` are already seeded). After that, **Step 7 — Campaign Strategy Workflow** (Margot, Gate 1/Gate 2) — and that's where Margot's Simon-roster wiring (deferred from Step 5) lands. The Step 3 parity gate remains independent.

**How to test the variant workflow in Studio (Step 6 verification):** apply the branch migrations, insert one `campaigns` row (with a `strategy` JSONB) and one `campaign_beats` row, then run the `variant` workflow with `{ campaignId, beatId, socialAccountId }` (use a seeded `social_accounts` id). It suspends at Gate 3; resume with `{ decision: 'approve' }` or `{ decision: 'request_change', instruction: '…' }`.
