...
| `sql/seed.sql` | The registry rows — 8 display metrics + the raw inputs feeding derived ones | Session 1 |
| `sql/views.sql` | `v_onchain_series`, `v_hash_ribbons`, `v_onchain_dashboard` (derivation lives here) | Session 1 |

> The `sql/` assets above were specced but never committed — the rows record what
...
(indexes, RLS, trigger). Apply `sql/views.sql` then `sql/seed.sql`.
...
style. **Apply order:** table DDL (in `schema.sql`) → `sql/views.sql` →
`sql/seed.sql`. If the project uses a migrations tool, copy these in rather than
