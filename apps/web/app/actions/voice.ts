...
function parseSnippet(formData: FormData) {
  ...
  return snippetSchema.safeParse({
    ...
    social_account_id: raw.social_account_id === '' || raw.social_account_id === undefined ? null : raw.social_account_id,
  });
}
...
export async function saveVoiceSnippet(formData: FormData) {
  ...
  // generated by the agents layer (the OpenAI key stays server-side there); a
  // snippet saved here is immediately listed/editable and becomes retrievable
  // once embedded. The scope is set on create only — it doesn't move on edit.
  const fields = {
...
export async function updateAccountGuidelines(accountId: string, guidelinesText: string) {
  ...
  const { error } = await supabase.from('account_feedback_guidelines').upsert({
