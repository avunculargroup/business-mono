...
import type { DocumentRow, DocumentVersionRow } from './DocsList';
import styles from './DocVersionEditor.module.css';
...
function versionToMarkdown(version: DocumentVersionRow): string {
...
export function DocVersionEditor({ document }: DocVersionEditorProps) {
  const sortedVersions = useMemo(
    () => [...document.document_versions].sort((a, b) => b.version_number - a.version_number),
  ...
  const handleSave = async () => {
    ...
    const result = await updateDocumentVersion(selectedVersion.id, { markdown: editorContent });
  ...
  const handleFork = async () => {
    ...
    const result = await createDocumentVersion(document.id, fd);
    setIsSubmitting(false);
    if (result.error) { error(result.error); return; }
  ...
  const handleApprove = async () => {
    if (!selectedVersion) return;
    setIsSubmitting(true);
    const result = await approveDocumentVersion(document.id, selectedVersion.id);
    setIsSubmitting(false);
    if (result.error) { error(result.error); return; }
    ...
    <div className={styles.root}>
      <div className={styles.toolbar}>
        ...
        {selectedVersion && (
          <StatusChip
            label={
              DOCUMENT_VERSION_STATUS_LABELS[selectedVersion.status as DocumentVersionStatus] ??
