...
function entityHref(change: EcosystemChange): string | null {
  if (change.productSlug) return `/products/${change.productSlug}`;
  if (change.advisorSlug) return `/advisors/${change.advisorSlug}`;
  return null;
}
...
export function SignalsView({ changes: initialChanges, watchHealth }: SignalsViewProps) {
  ...
  const [noteTarget, setNoteTarget] = useState<EcosystemChange | null>(null);
  ...
  const entities = useMemo(
    () => [...new Set(changes.map((c) => c.entityName).filter((n): n is string => !!n))].sort(),
    [changes],
  );
  const changeTypes = useMemo(
    () => [...new Set(changes.map((c) => c.changeType).filter((t): t is string => !!t))].sort(),
    [changes],
  );
  ...
  const shown = useMemo(() => {
    ...
    return changes
      .filter((c) => entity === ALL || c.entityName === entity)
      .filter((c) => changeType === ALL || c.changeType === changeType)
      .filter((c) => severity === ALL || c.severity === severity)
      .filter((c) => status === ALL || c.status === status)
      .filter((c) => !clientOnly || c.clientRelevant)
      .filter(
        (c) =>
          q === '' ||
  ...
  const patch = (id: string, fields: Partial<EcosystemChange>) =>
    setChanges((prev) => prev.map((c) => (c.id === id ? { ...c, ...fields } : c)));

  const handleAcknowledge = async (change: EcosystemChange) => {
    if (!change.id) return;
  ...
  const handleDismiss = async (change: EcosystemChange) => {
    if (!change.id) return;
  ...
  const handlePin = async (change: EcosystemChange) => {
    if (!change.id) return;
  ...
  const handleFlag = async (change: EcosystemChange) => {
    if (!change.id) return;
    const next = !change.clientRelevant;
    ...
    if ('error' in result) { error(result.error!); return; }
    patch(change.id, { clientRelevant: next });
  ...
  const handleSaveNote = async () => {
    ...
    if ('error' in result) { error(result.error!); return; }
    patch(noteTarget.id, { curatorNote: noteDraft || null });
    setNoteTarget(null);
    success('Note saved');
  };
                ...
                return (
                  <article key={change.id} className={styles.row}>
                    <div className={styles.rowMain}>
                        ...
                        <StatusChip
                          label={
                            ECOSYSTEM_CHANGE_TYPE_LABELS[
                              change.changeType as keyof typeof ECOSYSTEM_CHANGE_TYPE_LABELS
                            ] ?? change.changeType ?? 'Change'
                          }
                          color="neutral"
                        />
                        {href ? (
                          <Link href={href} className={styles.entity}>{change.entityName}</Link>
                        ) : (
                          <span className={styles.entity}>{change.entityName}</span>
                        )}
                      ...
                      {change.curatorNote && (
                        <p className={styles.curatorNote}>{change.curatorNote}</p>
                      )}
                      ...
                      <div className={styles.rowMeta}>
                        <span className={styles.mono}>
                          {change.occurredAt
                            ...
                            : change.detectedAt
                              ? `Detected ${formatRelativeDate(change.detectedAt)}`
                              : ''}
                        </span>
                        {change.watchLabel && <span>{change.watchLabel}</span>}
                        {change.complianceClass && change.complianceClass !== 'neutral' && (
                          <span>
                            {ECOSYSTEM_COMPLIANCE_CLASS_LABELS[
                              change.complianceClass as keyof typeof ECOSYSTEM_COMPLIANCE_CLASS_LABELS
                            ] ?? change.complianceClass}
                          </span>
                        )}
                    ...
                    <div className={styles.rowSide}>
                      ...
                      <div className={styles.actions}>
                        ...
                        <button
                          type="button"
                          className={styles.iconBtn}
                          onClick={() => { setNoteTarget(change); setNoteDraft(change.curatorNote ?? ''); }}
                        ...
                        <button
                          type="button"
                          className={`${styles.iconBtn} ${change.clientRelevant ? styles.iconBtnOn : ''}`}
                          onClick={() => handleFlag(change)}
                          aria-label={
                            change.clientRelevant
            ...
            watchHealth.map((watch) => (
              <div key={watch.id} className={styles.healthRow}>
                ...
                <div className={styles.rowMain}>
                  ...
                  <div className={styles.rowMeta}>
                    <span>{watch.entityName}</span>
                    <span className={styles.mono}>
                      {watch.lastCheckedAt
                        ? `Checked ${formatRelativeDate(watch.lastCheckedAt)}`
                        : 'Never checked'}
                    </span>
                    {(watch.consecutiveFailures ?? 0) > 0 && (
                      <span className={styles.mono}>
                        {watch.consecutiveFailures} consecutive failures
                      </span>
                    )}
                    {watch.ownerName && <span>{watch.ownerName}</span>}
                  </div>
                </div>
                <StatusChip
