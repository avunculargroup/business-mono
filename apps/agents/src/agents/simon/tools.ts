...
// Simon's per-turn delegation goes through Mastra's native subagent pattern,
// not hand-rolled delegate_to_<name> tools (see `agents` and
// `defaultOptions.delegation` on the Simon Agent in ../simon/index.ts).
// The Mastra core auto-generates an `agent-<name>` tool per registered
// subagent with nested span tracing and prompt-cache prefix sharing.
//
// The specialistAgents map above is still used here only for
// agentHealthCheck's deep-ping mode below.
