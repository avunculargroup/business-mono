...
function scheduleReconnect(reason?: string): void {
  ...
  console.log(
    `[rm-listener] ${scenario} — reconnect attempt ${reconnectAttempt} in ${delay / 1000}s` +
    (reason ? ` (${reason})` : '')
  );
 ...
 * When Simon dispatches to della, invokes della.generate() with the provided
 * message and logs the result back to agent_activity.
 */
export function startRelationshipManagerListener(): void {
  ...
  const channel = supabase
    .channel('relationship-manager-dispatches')
    .on(
      ...
      async (payload: { new: ActivityRow }) => {
        ...
        const dispatch = proposed.find((a) => a.agent === 'della');
        if (!dispatch) return;

        console.log(`[rm-listener] Dispatch received from activity ${row.id}`);

        const messages: CoreMessage[] = [{ role: 'user', content: dispatch.message }];

        let responseText: string;
        try {
          const result = await della.generate(messages);
          responseText = result.text;
        } catch (err) {
          console.error('[rm-listener] Della error:', err);
        ...
        console.log(`[rm-listener] Completed dispatch from activity ${row.id}`);
      }
    )
    .subscribe((status, err) => {
      if (channel !== currentChannel) return;

      console.log('[rm-listener] Subscription status:', status);
      if (err) console.error('[rm-listener] Subscription error:', err);
      if (status === 'SUBSCRIBED') {
        hasEverSubscribed = true;
        reconnectAttempt = 0;
        console.log('[rm-listener] Listening for Relationship Manager dispatches via Supabase Realtime');
